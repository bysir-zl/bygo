...
//原始方法 查询sql返回map
func (p *ModelFactory) Query(sql string, config DbConfig, args ...interface{}) (data []map[string]interface{}, err error) {

    data = nil
    dbDriver, err := Singleton(config)
...
func (p *ModelFactory) QueryToMap() (data []map[string]interface{}, err error) {
    ...
    dataMap, err2 := p.Query(sql, p.connect, args...)
    if err2 != nil {
        err = err2
        return
    }
...
func (p *ModelFactory) Count() (count int64, err error) {
    ...
    data, e := p.Query(sql, p.connect, args...)
    if e != nil {
        err = e
        return
    }
...
func (p *ModelFactory) Insert() (err error) {
    ...
    _, _insertId, _err := p.Exec(sql, p.connect, args...)
    if _err != nil {
        err = _err
        return
    }
...
func (p *ModelFactory) Update() (count int64, err error) {
    ...
    c, _, e := p.Exec(sql, p.connect, args...)
    if e != nil {
        err = e
        return
    }
...
func (p *ModelFactory)Exec(sql string, config DbConfig, args ...interface{}) (affectCount int64, lastInsertId int64, err error) {
